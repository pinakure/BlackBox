...
Sector::Sector(int x, int y, int z) {
	...
	this->subjective_y = y;
	this->subjective_z = z;	
	__int64 *dat = new __int64[Sector::size * Sector::size* Sector::size  ];
	__int64 *grs = new __int64[Sector::size * Sector::size* Sector::size  ];
	...
	bool enable_grass = int(TRUERAND(3)) > 1;
	...
	if (enable_grass) {
		...
		// Create trees!
		if (moisture > 60)
			terrain.createTrees(this->data, this->grass);
	}

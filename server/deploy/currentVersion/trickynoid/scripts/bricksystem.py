from debug      import debug, deprecate, error, exception, panic
from animation  import Animation
import vpu 

data = [
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
    [ 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ],
]

class BrickSystem:
    game    = None
    
    @staticmethod
    def initialize(game):
        BrickSystem.game = game
            
    def __init__(self):
        from data.scripts.brick import Brick
        self.bricks = []
        self.left   = 0
        for y in range(0, 26):
            for x in range(0, 14):
                self.bricks.append(Brick(x,y, data[y][x], 0))
    
    def update(self, delta):
        for b in self.bricks:
            # Check fall
            if b.isFalling:
                # b.fall()
                pass
        
    def loadBricks(self, map, width=14, height=26):
        deprecate('loadBricks', 'Use load instead')
        self.load(map, width, height)

    def load(self, map, width=14, height=26):
        self.bricks = []
        brickIndex = 0
        for y in range(0, height):
            for x in range(0, width):
                self.bricks[brickIndex] = Brick(x, y, map.getTileId(x, y, 0), map.getTileId(x, y, 2) -  0x81)
                self.brickIndex+=1
    
    def test(self, x, y):
        #x/=16;
        #y/=8;
        for b in self.bricks:
            if b.test(x, y):
                return b.isAlive()            
        return False
    
    def hit(self, x, y):
        brickX = x/16
        brickY = y/8
        brickId = (brickY * 14) + brickX
        
        causesBounce = False
        
        if brickX > 14: return True
        if brickX < 0 : return True
        if brickY > 25: return True
        if brickY < 0 : return True
        
        b = bricks[brickId]
        
        if b.isAlive():
            causesBounce = b.causesBounce()
            if not b.isSticked():
                b.fall()
            
            if b.isInverter():
                Brick.game.reaction.invertDeltas = True;#swapDeltas(game.reaction.target)
            
            if b.isDestructable():
                damage = Brick.game.reaction.target.getDamage()
                if damage == 9: causesBounce = False
                b.applyDamage(damage)
                print(f'Generate particle: {b.getGraph()}')
                Brick.game.getParticles().generate(b.getGraph(), x+2, y+2)
                Brick.game.getBackdrop().setColor(b.getGraph()-1)

            if b.getDamage() >= 9:
                Brick.game.addScore(b.getScore())                
                if b.getTokenType() > 0x00:
                    Brick.game.addToken(b.generateToken())
                b.kill()
        return self.causesBounce
    
    def getBricks(self):  
        return self.bricks    

    def get(self, index): 
        return self.bricks[index]
    
    def render(self):
        i = 0
        for y in range(0,26):
            for x in range(0, 14):
                b = self.bricks[i]
                if b.isAlive():
                    # Calculate brick screen absolute position
                    bx = 3 + ( b.getX()*16 )
                    by = 3 + ( b.getY()*8 ) + b.getOffset()
                    
                    # Draw Damage        
                    BrickSystem.game.map.set(x,y, 0x90+(b.getDamage()*(4/b.getHealth())), 1)
                    
                    # Draw Brick
                    if b.getGraph() < 0x9C:
                        # Draw Shadow
                        BrickSystem.game.map.set(x,y, 0x90, 1)
                    
                        # Draw brick (with damage and shadow)
                        BrickSystem.game.map.set(x,y, b.getGraph()-1, 0)
                    else:
                        BrickSystem.game.map.set(x,y, b.getGraph()-1, 0)
                i+=1
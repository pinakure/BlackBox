var Triste = {
    ...
    /*-----------------------------------------------------------------------*/
    findallmovements: function(){
        var movements = [];
        var scores = Triste.map.getScores();
        var p = Triste.currentpiece;
        var p_angle = p.rotation;
        for(angle=0; angle<4; angle++){
            p.target_angle = angle;
            p.followAngle();
            var landing_zones = Triste.map.getLandingZones(p);
            var penalties = Triste.map.getPenalties(p, landing_zones);
            var lines = Triste.map.getPosibleLines(p, landing_zones);
            for(i=0; i<landing_zones.length; i++){
                movements[movements.length] = [ angle, landing_zones[i], scores[landing_zones[i]], penalties[i], lines[i], scores[landing_zones]+(lines[i]-penalties[i])];
            }
        }
        p.target_angle = p_angle;
        p.followAngle();
        return movements;
    },
    /*-----------------------------------------------------------------------*/
    findbestmovement: function(){
        var movement = [];
        // evaluate movements
        var movements = Triste.findallmovements();
        ...
        for(mi in movements){
            ...
            if(movement[5] > hiscore){
                hiscore = movement[5];
                best_movement = mi;
            }
        }
        return movements[best_movement>-1?best_movement:parseInt(Math.random()*movements.length)];
    },
    /*-----------------------------------------------------------------------*/
    logic           : function(){
        var p = Triste.currentpiece;
        switch( p.logic() ){
            case Triste.GAMEOVER:
                ...
                case Triste.NEWSHAPE:
                    Triste.newshape();
                    var p = Triste.currentpiece;
                    var movement = Triste.findbestmovement();
                    p.target_angle = movement[0];
                    p.target = movement[1];
                    print(p.toTable(), 'pieces');
                    break;
                default:
                case Triste.CONTINUE:
                    print(Triste.map.toTable(true, Triste.map.getScores()), 'map', true);
                    break;
            }

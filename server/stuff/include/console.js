...
Console.prototype.drawCursor = function(){
	...
	/*var pos = (this.currentLine.length/{$width/($tile_width/2)})*{$width/($tile_width/2)};*/
	var len = this.len(this.currentLine);
	var width = parseInt({$width/($tile_width/2)});
	var lines = parseInt(len / width);
	var pos = len - (lines > 0 ? lines * width : 0);
	
	px = pos * {$tile_width};
	py = {($height) - $tile_height};
	if(vpu.console.clock&7 > 3)vpu.ctx.drawImage(vpu.data0,2*{$tile_width}, {$tile_height}<<0,{$tile_width},{$tile_height},px>>1,py,{$tile_width}>>1,{$tile_height});
}
...
Console.prototype.dump = function() {
	...
	this.redraw = false;	
	var i;
	for(i={($width / $tile_width)*($height / $tile_height)}-{$width / $tile_width}; i<{($width / $tile_width)*($height / $tile_height)}; i++){
	...
	var lineCount = parseInt(this.currentLine.length / {$width/($tile_width/2)});
	
	vpu.locateCursor(0, i);
	vpu.print(this.currentLine.substring(lineCount*{$width/($tile_width/2)}) ,0x2);	
...
Console.prototype.init = function() {
	...
	for(i=0; i<{$height / $tile_height}-1; i++){
		this.lines[i]="";
		this.lineCount++;
	}
...
Console.prototype.print = function(text) {
	...
	var lineWidth = {$width/($tile_width/2)};
